...
class KaiLorebook(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.config = config
        self.owner_id = str(config.get("owner_id", ""))
    ...
    @filter.llm_tool(name="lorebook_write")
    async def lorebook_write(self, event: AstrMessageEvent, action: str, name: str = "", content: str = "", keywords: str = "", priority: int = 50):
        '''管理世界书条目（仅限主人）。当你想记住关于宝宝、关系、重要事件的长期信息时调用。

        Args:
            action(string): 操作类型，必填。可选值：add（新增）、update（更新）、delete（删除）、search（搜索）
            name(string): 条目名称。add/update/delete必填；search时作为搜索关键字
            content(string): 条目正文内容。add必填，update可选
            keywords(string): 触发关键词，多个用英文逗号分隔。可选，留空则用name作为关键词
            priority(number): 优先级，数字越小越优先，默认50
        '''
        ...
        try:
            ...
            if action == "add":
                ...
                pri = priority if priority > 0 else 50
                self.entries.append({
                    "name": name, "enabled": True, "priority": pri,
                    "keywords": kw_list or [name], "content": content,
                })
                self.entries.sort(key=lambda e: e["priority"])
                self._save_entries()
                return f"added: {name}"
            if action == "update":
                for e in self.entries:
                    if e["name"] == name:
                        ...
                        if kw_list: e["keywords"] = kw_list
                        if priority and priority != 50: e["priority"] = priority
            ...
            if action == "delete":
                ...
                return f"not found: {name}"
            if action == "search":
                q = (name or content or "").lower()
                hits = [e["name"] for e in self.entries if q in e["name"].lower() or q in e["content"].lower()]
                return "hits: " + ", ".join(hits) if hits else "no hits"
            return "unknown action"
        except Exception as ex:
            logger.error(f"lorebook_write error: {ex}", exc_info=True)
            return f"error: {ex}"
